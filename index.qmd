...
other-links:
  - text: Example High Level Risk assessment
    href: fig/HighLevelRiskAssessment.docx
    icon: file-word
    target: "_blank"
  - text: Example Vendor Assessment Statement
    href: fig/VendorAssessmentStatement.docx
    icon: file-word
    target: "_blank"
  - text: Example Validation plan and test plan
    href: fig/ValidationAndTestPlan.docx
    icon: file-word
    target: "_blank"
  - text: Example Validation plan and test report
    href: fig/ValidationAndTestReport.docx
    icon: file-word
    target: "_blank"
  - text: Example User Requirement specification
    href: fig/UserRequirementSpecification.docx
    icon: file-word
    target: "_blank"
execute:
  echo: false
...
|              | Name             | Date       |
|--------------|------------------|------------|
| Authored by: | Cyril Jaksic[^1] | 2026-05-07 |
| Reviewed by: | Alan Haynes[^2]  | 2026-05-07 |

`#set align(left)`{=typst}

[^1]: Statistician, Clinical Research Centre, Geneva University Hospitals
...
+:======================+:======================+:======================+:======================+
| 1.0                   | 2025-01-01            | Michael Coslovsky,    | Initial draft         |
|                       |                       | Christine Otieno,     |                       |
|                       |                       | Alan Haynes           |                       |
+-----------------------+-----------------------+-----------------------+-----------------------+
| 1.1                   | 2026-05-07            | Cyril Jaksic          | Update in links and   |
|                       |                       |                       | documents version     |
+-----------------------+-----------------------+-----------------------+-----------------------+
| 1.2                   | 2026-07-13            | Charlotte Michelud    | Adding the reviewer   |
|                       |                       |                       | assignment process    |
+-----------------------+-----------------------+-----------------------+-----------------------+
...
This policy provides the framework for validating R and R add-on packages in use by Swiss Clinical Trial Organization (SCTO) and its SCTO network members[^5] for the purpose of creating R products in the context of clinical research. Validation may be executed on SCTO or CTU level, where the SCTO provides
...
[^5]: for purposes of this document referred to as CTUs = Clinical Trial
    Units

Validation activities in general ensure that computerised systems can be relied upon to consistently produce a product that meets predetermined specifications of accuracy. The formal process of validation results in a validation documentation set[^6].

[^6]: A validation documentation set is prepared to provide evidence of
...
According to international standards and guidelines listed below, any GxP[^7] critical system supporting activities such as statistical analysis (e.g., in the context of clinical studies) should be validated:

[^7]: GxP processes refer to Good Practice standards, including Good
...
-   [GAMP 5: A Risk Based Approach to Compliant GxP Computerized systems](https://ispe.org/topics/gamp)
...
This policy is based on a business processes risk assessment provided by the [SCTO statistics' platform](https://swissclinicaltrialorganisation.github.io/validation_platform/sop/businessproc_ra.html).

# Background  {#sec-background}

R is a 'language and environment for statistical computing and graphics' (source: [r-project.org](https://www.r-project.org/about.html)). It is an official part of the Free Software Foundation's GNU[^8] project and is released under the [Free Software Foundation's GNU Public License](https://www.gnu.org/licenses/gpl-3.0.html) . Clinical Trial Units (CTU; see definition below) use R primarily, but not exclusively, in the framework of clinical research for tasks including sample size estimation and statistical design consideration, as well as the preparation and analysis of data from supported studies. Internal use for other reporting, visualisation and business management might also take place, according to individual CTU needs, but is not the focus of this policy.

[^8]: The GNU Project is a free software, mass collaboration project
...
-   **IT Infrastructure level** (IT infrastructure qualification
    *according to **local CTU processes**[^9], see @sec-it-iq*):
...
[^9]: For example, standards set by the local IT department.
...
|                    |   activity and how this aspect can be    |   activity is NOT          |
|                    |    covered for R (suggested R specific   |   required for R: a        |
|                    |    document/activity)*                   |   rationale, why this      |
|                    |                                          |   document/activity can    |
|                    |                                          |   be omitted*              |
|                    | - *Link to example                       |                            |
...
|                    |                                          |   periodic review          |
|                    | - [Example](                             |   cycles. The review       |
|                    |   fig/HighLevelRiskAssessment.docx)      |   process should be        |
...
| **Vendor           | the assess reliability of a system       | - Yes                      |
| Assessment**       | vendor. This may be done based on an     |                            |
|                    | audit or review of relevant vendor       | **Frequency of update:**   |
|                    | documentation.                           |                            |
|                    |                                          | - Created before/ with     |
|                    | ***R base validation specific            |   the first R base         |
|                    | approach***: Since R is an open-source   |   installation             |
|                    | software managed by a consortium, a      |   validation.              |
|                    | traditional vendor assessment/ audit     |                            |
|                    | approach is not feasible. A critical     | - Updated, if any of the   |
|                    | review of "[R: Regulatory Compliance     |   assessed aspects         |
|                    | and Validation Issues - A Guidance       |   change, specifically,    |
|                    | Document for the Use of R in Regulated   |   with a publication of    |
|                    | Clinical Trial Environments](            |   a new article version    |
|                    | https://www.r-project.org/doc/R-FDA.pdf)"|   of the article           |
|                    | may suffice as vendor assessment and     |   mentioned above, or in   |
|                    | should be documented together with any   |   case the R consortium    |
|                    | gaps identified that may require         |   replaces the existing    |
|                    | attention during the validation of R and |   document.                |
|                    | its R add-on packages.                   |                            |
|                    |                                          | - Reviewed during          |
|                    |                                          |   periodic review cycles   |
|                    |  **Example:**                            |   and review documented,   |
|                    |                                          |   if no updates are        |
|                    |  - [Example](                            |   required.                |
|                    |   fig/VendorAssessmentStatement.docx)    |                            |
+--------------------+------------------------------------------+----------------------------+
| R Base             | **Purpose:**                             | **Required for R base      |
| installation       |                                          | installation validation:** |
|                    | The purpose of such a                    |                            |
...
|                    | fig/ValidationAndTestPlan.docx)          |       for a newly          |
|                    |                                          |       released version.    |
|                    |                                          |       Instead, it would    |
|                    |                                          |       contain generic      |
|                    |                                          |       rules for test       |
...
|                    | - [Example](                             |   OR if any of the         |
|                    |  fig/UserRequirementSpecification.docx)  |   existing requirements    |
...
| R Base             | **Purpose:**                             | **Required for R base      |
| installation       |                                          | installation validation:** |
|                    | The purpose of such a document would be  |                            |
| **Software         | to provide a process to follow during    | - No (see above for how    |
| Installation       | the installation of a software. It is    |   this is covered)         |
| Plan/Instruction** | typically provided either by the vendor  |                            |
|                    | or prepared by local IT departments.     | **Frequency of update:**   |
|                    |                                          |                            |
|                    |  ***R base validation specific           | - N/A                      |
|                    |  approach:*** It should be sufficient to |                            |
|                    |  follow the instructions on CRAN when    |                            |
|                    |  downloading and installing the new R    |                            |
|                    |  base installation version.              |                            |
|                    |                                          |                            |
|                    | **Important** is that the installation   |                            |
|                    | of the R base installation and version   |                            |
|                    | management follow the CTU's local        |                            |
|                    | processes for validated systems and that |                            |
|                    | all versions installed are documented,   |                            |
|                    | including the date/time of installation  |                            |
|                    | (see Installation Verification Document  |                            |
|                    | below).                                  |                            |
+--------------------+------------------------------------------+----------------------------+
| R Base             | **Purpose:**                             | **Required for R base      |
| installation       |                                          | installation validation:** |
...
|                    |                                          |   Specification, this is   |
|                    | - _An example will be available soon_    |   a good indicator to      |
|                    |                                          |   also review the          |
|                    |                                          |   existing test            |
|                    |                                          |   protocols.               |
|                    |                                          |                            |
...
| installation       |                                          | installation validation:** |
|                    | The purpose of such a document is to     |                            |
| **Software         | provide evidence of the installation     | - Yes                      |
| Installation       | of a specific system version on an       |                            |
| Verification       | environment/ machine that is used for    | **Frequency of update:**   |
| document (TEST)**  | testing purposes before the new system   |                            |
|                    | version is released to production.       | - To be created with       |
|                    |                                          |   every new R base         |
|                    | ***R base validation specific            |   installation version     |
|                    | approach***: In case of R this may be a  |   to be validated for      |
|                    | separate document or an entry in an R    |   productive use           |
|                    | repository or other "version tracker"    |                            |
|                    | (depending on your local IT processes).  | - No periodic review       |
|                    | The following information should be      |   required                 |
|                    | captured:                                |                            |
|                    |                                          |                            |
...
|                    |   (may be someone from your local IT     |                            |
|                    |   IT department)                         |                            |
|                    |                                          |                            |
|                    | - Outcome of installation: successful,   |                            |
|                    |   successful with deviations, not        |                            |
|                    |   successful                             |                            |
|                    |                                          |                            |
|                    | - If applicable: Any actions taken in    |                            |
|                    |   addition to the regular "download and  |                            |
|                    |   install from CRAN" process (e.g.,      |                            |
|                    |   uninstalling an older version)         |                            |
|                    |                                          |                            |
|                    | **Note:** depending on your local        |                            |
|                    | processes it may not be feasible or even |                            |
|                    | possible to do a preliminary "test       |                            |
|                    | installation" and perform tests before   |                            |
|                    | productive use of the new R base         |                            |
|                    | installation version (e.g., if R is      |                            |
|                    | distributed remotely by your local IT    |                            |
|                    | department). In this case, we recommend  |                            |
|                    | you still execute the tests after the    |                            |
|                    | installation and document the test       |                            |
|                    | execution. Should any issues be detected |                            |
|                    | during testing, please inform your local |                            |
|                    | IT department immediately. Ideally you   |                            |
|                    | implement a process that would ensure    |                            |
|                    | nobody is using the new R version for    |                            |
|                    | productive use before the tests are      |                            |
|                    | completed successfully (i.e., without    |                            |
|                    | major issues) and the validation & test  |                            |
|                    | report is approved (see "Validation &    |                            |
|                    | Test Report" below).                     |                            |
|                    |                                          |                            |
|                    | **Example:**                             |                            |
|                    |                                          |                            |
|                    | - see [Example for Validation & Test     |                            |
|                    |   Report available](                     |                            |
|                    |   fig/ValidationAndTestReport.docx)      |                            |
+--------------------+------------------------------------------+----------------------------+
| R Base             | **Purpose:**                             | **Required for R base      |
| installation       |                                          | installation validation:** |
|                    | The purpose of such a document is to     |                            |
| **Executed test    | provide evidence for the execution of    | - Yes                      |
| protocols ("User   | the pre-defined test protocols for each  |                            |
| Acceptance         | new R base installation version (i.e.,   | **Frequency of update:**   |
| Tests")**          | follow the steps described in the test   |                            |
|                    | protocols and document the outcome       | - To be created with       |
|                    | according to the CTU's local processes). |   every validation of a    |
|                    |                                          |   new R base               |
|                    | **Example:**                             |   installation version.    |
|                    |                                          |                            |
|                    | - _An example will be available soon_    | - No periodic review       |
|                    |                                          |   required                 |
+--------------------+------------------------------------------+----------------------------+
| R Base             | **Purpose:**                             | **Required for R base      |
| installation       |                                          | installation validation:** |
|                    | The purpose of such a document is to     |                            |
| **Traceability     | ensure and provide evidence that all     | - Yes                      |
| Matrix**           | user requirements are tested during the  |                            |
|                    | validation process. You may omit testing | **Frequency of update:**   |
|                    | low risk requirements, if such an        |                            |
|                    | approach is described in the Validation  | - Created with the first   |
|                    | & Test Plan and complies to your CTU's   |   R base installation      |
|                    | processes.                               |   validation.              |
|                    |                                          |                            |
|                    | Traceability between requirements and    | - Updated, if the user     |
|                    | tests may be achieved by creating a      |   requirements             |
|                    | separate document (traceability matrix)  |   specification and or     |
|                    | or by linking requirements and tests     |   the test protocols are   |
|                    | within the requirements and tests        |   updated.                 |
|                    | themselves. It is important, that        |                            |
|                    | whatever means is used, it is feasible   | - Reviewed during          |
|                    | to proof that all requirements are       |   periodic review cycles   |
|                    | verified with a test (or if not tested   |   and review documented,   |
|                    | formally, a rationale why the test is not|   if no updates are        |
|                    | required, e.g., low risk requirements    |   required.                |
|                    | which can be considered "verified" based |                            |
|                    | on the experience by the user community).|                            |
|                    |                                          |                            |
|                    | **Example:**                             |                            |
|                    |                                          |                            |
|                    | - _An example will be available soon_    |                            |
+--------------------+------------------------------------------+----------------------------+
| R Base             | **Purpose:**                             | **Required for R base      |
| installation       |                                          | installation validation:** |
|                    | The purpose of such a document is to     |                            |
| **Validation       |                                          | - Yes                      |
| & Test Report**    | - summarize the testing activities and   |                            |
|                    |   results, including any deviations      | **Frequency of update:**   |
|                    |   from the expected results              |                            |
|                    |                                          | - To be created with       |
|                    | - document all completed validation      |   every new R base         |
|                    |   activities, including any deviations   |   installation version     |
|                    |   from the original plan (see Validation |   validated for            |
|                    |   & Test Plan). Deviations should be     |   productive use           |
|                    |   justified and assessed for             |                            |
|                    |   criticality.                           | - No periodic review       |
|                    |                                          |   required                 |
|                    | - document the acceptance for productive |                            |
|                    |   use (with or without restrictions)     |                            |
|                    |                                          |                            |
|                    | The exact content of this report depends |                            |
|                    | on the CTU's local processes. Separate   |                            |
|                    | documents may be created to summarize    |                            |
|                    | the testing activities and results       |                            |
|                    | (Test Report) and the overall executed   |                            |
|                    | validation against the plan, incl. a     |                            |
|                    | statement of acceptance of an R base     |                            |
|                    | installation version for productive use  |                            |
|                    | (Validation Report).                     |                            |
|                    |                                          |                            |
|                    | **Example:**                             |                            |
|                    |                                          |                            |
...
|                    | of R add-on packages and      |                            |
|                    | functions therein, shall be   |                            |
|                    | documented in the Statistical |                            |
|                    | Analysis Plan, a Statistical  |                            |
|                    | Protocolor other, similar     |                            |
|                    | documentation[^10].           |                            |
|                    |                               |                            |
|                    | **Relevant SCTO               |                            |
|                    | Documentation:**              |                            |
...
| Assessment**       | document would be to specify  | - No (covered by the R     |
|                    | the risks related to a        |   add-on Package High-level|
|                    | specific function.            |   Risk assessment)         |
|                    |                               |                            |
|                    | ***R add-on package specific  | **Frequency of update:**   |
|                    | approach***: We take a        |                            |
|                    | package level approach -      | - NA                       |
|                    | the assessed baseline risk of |                            |
|                    | the R add-on package defines  |                            |
|                    | the risk of all functions     |                            |
|                    | included within.              |                            |
|                    |                               |                            |
|                    | **Relevant SCTO SOP:**        |                            |
|                    | - [SCTO R add-on Package      |                            |
...
[^10]: Examples of such other documents could be the defined research
...
assessed, documented, and managed on all those levels, specifically on:
...
[Guideline on computerised systems and electronic data in clinical trials](https://www.ema.europa.eu/en/documents/regulatory-procedural-guideline/guideline-computerised-systems-and-electronic-data-clinical-trials_en.pdf) 
: European Medicines Agency: Guideline on computerised systems and electronic data in clinical trials (2023)


[ICH E6: Guideline for Good Clinical Practice](https://database.ich.org/sites/default/files/E6_R2_Addendum.pdf)
: International Council for Harmonisation, E6 Good Clinical Practice (1997) and Integrated Addendum to Good Clinical Practice (GCP) (2016)


[The Good Automated Manufacturing Practice (GAMP) Guide 5: A Risk Based Approach to Compliant GxP Computerized Systems (ISPE/GAMP, 2008)](https://ispe.org/publications/guidance-documents/gamp-5-guide-2nd-edition)
...
[FDA 21 CFR part 11: Electronic Records; Electronic Signatures](https://www.ecfr.gov/cgi-bin/text-idx?SID=bb609049963efdb7e40b991244ba59e1&mc=true&tpl=/ecfrbrowse/Title21/21cfr11_main_02.tpl)
: FDA regulation: Title 21---Food and Drugs, Chapter I - Food and Drug Administration, Department of Health and Human Services, Subchapter A - General, Part 11 --- Electronic Records; Electronic Signatures

[General Principles of Software Validation; Final Guidance for Industry and FDA Staff (2002)](https://www.fda.gov/regulatory-information/search-fda-guidance-documents/general-principles-software-validation)
: FDA guidance paper for computerised system validation.

[EudraLex Volume 4: Good Manufacturing Practice (GMP) guidelines, Annex 11: Computerised Systems](https://health.ec.europa.eu/medicinal-products/eudralex/eudralex-volume-4_en#annexes) 
: European standard for comupterised systems validation. 
