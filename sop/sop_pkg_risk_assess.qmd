...
|         |            | Elio Carreras      |                                     |
+---------+------------+--------------------+-------------------------------------+
| 1.1     | 2026-09-22 | Jakob Heimer       | Metrics 5 and 8: formulas stated as |
|         |            |                    | implemented. Calculation unchanged. |
+---------+------------+--------------------+-------------------------------------+
...
5.  **Number of dependencies** (`n_dependencies`): This metric assesses
    ...
    'riskmetric' package (R validation hub, 2023), we use the classic
    logistic curve
    $s(x) = 1/\ (1 + exp( - k\left( x - x\lbrack 0\rbrack \right))$ with
    a sigmoid midpoint of 4 dependencies, i.e., x\[0\] = 4, and a
    logistic growth rate of k = 0.5, rescaled so that 0 dependencies
    give a score of 0: $score = (s(x) - s(0)) / (1 - s(0))$.
...
8.  **Number of downloads in the last year** (`nr_downloads_12_months`):
    ...
    the approach taken by the 'riskmetric' package (R validation hub,
    2023) and use its simplification of the logistic curve
    $score = 1.5\ /\ (x / 100000 + 1.5)$, where x is the number of
    downloads in the last 12 months (score 0.5 at 150,000 downloads).
